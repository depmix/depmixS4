...
setMethod("logLik",signature(object="depmix"),
	...
	function(object,method=c("fb","lystig","classification"),na.allow=TRUE) { 
		...
		if(method=="fb") ll <- fb(init=object@init,A=object@trDens,B=object@dens,ntimes=object@ntimes,stationary=object@stationary,na.allow=na.allow)$logLike
...
setMethod("logLik",signature(object="mix"),
	...
	function(object,method=c("fb","lystig","classification"),na.allow=TRUE) {
		...
		if(method=="classification") {
		    ntimes <- ntimes(object)
		    gamma <- fb(init=object@init,A=matrix(0,1,1),B=object@dens,ntimes=ntimes,stationary=TRUE)$gamma
