...
setMethod("logLik",signature(object="depmix"),
	function(object,method="lystig") { 
		...
		if(method=="Cfb") ll <- Cfb(object@init,object@trDens,object@dens,object@ntimes,object@stationary)$logLike
...
setMethod("logLik",signature(object="mix"),
	function(object,method="lystig") { 
		...
		if(method=="Cfb") ll <- lystig(object@init,matrix(0,1,1),object@dens,object@ntimes,TRUE)$logLike		
		attr(ll, "df") <- freepars(object)
		attr(ll, "nobs") <- nobs(object)
		class(ll) <- "logLik"
		ll
